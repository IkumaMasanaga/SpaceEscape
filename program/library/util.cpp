...
namespace t2k {
	...
	std::vector<std::string> split(const std::string& string, const char ch) {

		std::vector<std::string> ret;
		std::string str = string;

		while (true) {
			int count = str.find(ch);
			...
			std::string elem = str.substr(0, count);
	...
	int randomRangeI(const int min, const int max) {
